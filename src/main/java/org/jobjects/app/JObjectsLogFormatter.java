  ...
  public String format(LogRecord record) {
    ...
    if (SystemUtils.IS_OS_UNIX | SystemUtils.IS_OS_WINDOWS_10) {
      if (Level.FINEST.equals(record.getLevel())) {
        output.append(StringUtils.rightPad("[" + ANSI_GREEN + record.getLevel()
                                                                    .getName() + ANSI_RESET + "]", 9));
        ...
        output.append(StringUtils.rightPad("[" + ANSI_PURPLE + record.getLevel()
                                                                     .getName() + ANSI_RESET + "]", 9));
      } else if (Level.INFO.equals(record.getLevel())) {
        output.append(StringUtils.rightPad("[" + ANSI_BLUE + record.getLevel()
                                                                   .getName() + ANSI_RESET + "]", 9));
      } else if (Level.WARNING.equals(record.getLevel())) {
        output.append(StringUtils.rightPad("[" + ANSI_YELLOW + record.getLevel()
                                                                     .getName() + ANSI_RESET + "]", 9));
      } else if (Level.SEVERE.equals(record.getLevel())) {
        output.append(StringUtils.rightPad("[" + ANSI_RED + record.getLevel()
                                                                  .getName() + ANSI_RESET + "]", 9));
      } else {
    ...
    output.append(Thread.currentThread()
          ...
          .append(" : ")
          .append("..."
              + StringUtils.substringAfterLast(record.getSourceClassName(),
              "org.jobjects.") + "." + record.getSourceMethodName() + "()")
          .append(" : ");
